...
import Data.Attoparsec.ByteString ( parseOnly, option, Parser, count, string, take )
import Data.Char ( ord )
import Data.ByteString ( ByteString )
import Data.FIX.Arbitrary (arbitraryByteString)
import qualified Data.ByteString.Char8 as C ( pack )
import Data.Maybe ( fromMaybe )
import qualified Data.LookupTable as LT ( new, lookup, insert )
...
tagsP ts = option LT.new (insertValue LT.new)
        where
           ...
           nextTag = do
                tag' <- toTag
                maybe (fail "") return $ LT.lookup tag' ts
...
groupP spec = do n <- toInt           -- number of submessages
    ...
    let headerP'  = tagsP $ fsHeader spec  -- parse header
        ...
        fixP' = do
                ...
                return FIXMessage
                    ...
                    , mTrailer = t }
    in
        either fail return $ parseOnly fixP' msg
...
tBeginString = FIXTag
    ...
    , tnum = 8
    , tparser = toFIXString
    , arbitraryValue = FIXString <$> arbitraryByteString }
